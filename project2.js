...
class MeshDrawer {
	...
	draw(trans) {
		...
		// Clamp the lightX and lightY values to the range [-20, 20]
		lightX = Math.max(-10, Math.min(10, lightX));
		lightY = Math.max(-10, Math.min(10, lightY));

		// Pass the clamped light position to the shader
...
setTexture(img) {
    ...
    if (isPowerOf2(img.width) && isPowerOf2(img.height)) {
        ...
        // Handle non-power-of-two sized textures
		/**
